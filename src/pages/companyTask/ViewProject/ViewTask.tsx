...
export default function CompanyTaskView() {
  ...
  const resourceCompanyId = taskApiData?.data?.companyId;
  const isAuthorized =
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      ...(isAuthorized && taskApiData?.data?.taskName
        ? [
            {
              label: `${taskApiData.data.taskName}`,
              href: "",
              isHighlight: true,
            },
          ]
  ...
  }, [setBreadcrumbs, isAuthorized, taskApiData?.data?.taskName]);
  ...
  return (
    <CompanyAccessGuard companyId={resourceCompanyId} isLoading={!taskApiData}>
