...
export default function RepeatMeeting() {
  ...
  const { data: meetingTiming } = useGetRepeatMeetingById(
    repetitiveMeetingId ?? "",
  );
  ...
  const resourceCompanyId = meetingTiming?.companyId;
  const isAuthorized =
  ...
  }, [meetingName, setBreadcrumbs, isAuthorized]);
  ...
  const handleAddEmp = async (data: EmployeeDetails[]) => {
    if (repetitiveMeetingId && meetingTiming?.repetitiveMeetingId) {
      const payload = {
                                ...
                                {meetingStatus !== "NOT_STARTED" &&
                                  meetingStatus !== "ENDED" && (
                                    <div>
                                      <DropdownMenu
                                        open={
                                          dropdownOpen === note.meetingNoteId
                                        }
                                        onOpenChange={(open) =>
                                          setDropdownOpen(
