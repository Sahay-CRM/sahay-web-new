...
import type { CompanyLogin, Login } from "@/features/types";
...
const useLogin = () => {
  ...
  const [companies] = useState<CompanyLogin[]>([]);
  ...
  const onSubmit = async (data: Login) => {
    ...
    if (!statusSentOtp) {
      sendOtp(
        { mobile: countryCode + data.mobile, userType: data.userType },
        {
          onSuccess: (response) => {
      ...
      verifyOtp(
        { mobile: countryCode + data.mobile, otp: data.otp!, userType: data.userType },
        {
          onSuccess: (response) => {
            if (response.status) {
